            ...
            "quote": {
                ...
                func: async function (msg, db) {
                    ...
                    if (targetText.startsWith("http")) {
                        try {
                            var split = targetText.split("discordapp.com/channels/")[1].split("/").filter((el) => el != null && el != undefined && el.length > 0);
                            if (split.length == 0)
                                throw TypeError;
                            ...
                            if (guild != undefined) {
                                ...
                                if (channel != undefined) {
                                    if (channel.type != "text") {
                                        msg.channel.send("Unable to quote message. The provided channel is not a text channel :(");
                                        return;
                                    }
                                    ...
                                    if (message != undefined) {
                                        ...
                                        msg.channel.send("Unable to quote message. I couldn't find the target message :(");
                                        return;
                                    }
                                } else {
                                    msg.channel.send("Unable to quote message. I couldn't find the channel that message is from :(");
                                    return;
                                }
                            } else {
                                msg.channel.send("Unable to quote message. I'm not in the server where that message came from :(");
